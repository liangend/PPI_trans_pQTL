library(data.table)
setwd('/project/xuanyao/jinghui/pqtl/03_p/ukb_random/')
input_p = list.files('./', pattern = '.rds')
for (i in 1:length(input_p)) {
  ...
  if(!is.null(tmp_y) & length(tmp_y) > 0){
    ...
    fwrite(tmp_p, '/project/xuanyao/jinghui/pqtl/04_fdr/ukb/pco_small_p_random.txt', 
           sep = '\t', append = T)
  }
  if (i %% 500 == 0) {
    print(i)
