...
## INTERVAL prot list
prot_list = fread('pqtl/05_h2/00_ref/Sun_2018_prot_w_coor.txt')

## number of SNP in each annotation for each chromosome
cis_snp = c()
trans_1mb_snp = c()
trans_5mb_snp = c()
for (i in 1:nrow(prot_list)) {
  file_i = prot_list$target[i]
  n_snp = fread(paste0('/project/xuanyao/jinghui/pqtl/05_h2/03_n_snp/Sun/', file_i, '.txt'))
...
cis_files = list.files('pqtl/05_h2/02_h2_results/Sun_h2/cis_1mb/', 
                       pattern = 'results')
coeff_cis = c()
for (i in cis_files) {
  h2_i = fread(paste0('pqtl/05_h2/02_h2_results/Sun_h2/cis_1mb/', i))
...
## obtaine the trans-coefficients (per-snp h2 for each annotation) across all proteins
trans_files = paste0(prot_list$target, '_h2.results')
coeff_trans = c()
for (i in trans_files) {
  h2_i = fread(paste0('pqtl/05_h2/02_h2_results/Sun_h2/trans_5mb/', i))
  coeff_i = h2_i$Coefficient
...
prot_list$cis_1mb_h2 = cis_h2
prot_list$trans_1mb_h2 = trans_1mb_h2
prot_list$trans_1mb_h2_ind = trans_1mb_h2_ind
prot_list$trans_5mb_h2 = trans_5mb_h2
prot_list$trans_5mb_h2_ind = trans_5mb_h2_ind
fwrite(prot_list, 'pqtl/05_h2/04_h2_summ/prot_h2_sun_1mb_5mb.txt', sep = '\t')
