...
#file.z = paste0('/project/xuanyao/jinghui/pqtl/01_zmat/mod1.chr1.txt.gz')
...
#file.p = '/project/xuanyao/jinghui/pqtl/03_p/p.mod1.chr1.rds'
...
gene_in_mod = data.frame(gene = colnames(z.mat))
gene_w_pos = unique(merge(gene_in_mod, prot_meta[, c('gene', 'chr')]))
# remove proteins/protein complex on the same chromosome as the snps
chr_set = sapply(strsplit(gene_w_pos$chr, ';', fixed = T), '[')
cis_set = sapply(chr_set, function(x){sum(x == paste0("chr", chr))})
gene_trans = gene_w_pos$gene[cis_set == 0]
