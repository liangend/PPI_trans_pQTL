...
dgn_cis_gene = dgn_cis_gene[dgn_cis_gene$phenotype_id %in% ukb_prot$gene_name, ]
...
file_chr_read = file_chr_all[grep(paste0(chr_i, '_'), file_chr_all)]
...
dgn_cis = cbind(dgn_cis, 
                summ_stats_j[match(dgn_cis$variant_id, ukb_hg19_j), 
                             c('ALLELE0', 'ALLELE1', 'A1FREQ', 'BETA', 'SE', 'LOG10P')])

fwrite(dgn_cis, paste0('pqtl/12_beta_across_two_data/ukb_beta_dgn_cis/', i, '.txt'),
       sep = '\t', col.names = F)
