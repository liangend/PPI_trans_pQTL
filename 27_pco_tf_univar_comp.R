...
pco_loci = fread('pqtl/04_fdr/ukb/pco_tf_loci_nonoverlap.txt')
pco_loci = as.data.frame(pco_loci)
done_loci = fread('pqtl/04_fdr/ukb/pco_tf_univar_comp.txt')
colnames(done_loci) = c(colnames(pco_loci), 'max_univar_logP')

pco_loci = pco_loci[-which(pco_loci$loci %in% done_loci$loci), ]

prot_mod = fread('pqtl/11_prot_complex/tf_mod.txt')
...
for (j in 1:length(mod_i)) {
  ...
  loci_j$max_univar_logP = apply(minp_all, 2, max)
