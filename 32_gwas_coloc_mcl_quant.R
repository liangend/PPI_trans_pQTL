...
gwas_loci_files = list.files('gtex/10_GWAS_coloc/GWAS_sum_stats/quant_trait/gwas_loci_500kb/')
gwas_loci = fread(paste0('gtex/10_GWAS_coloc/GWAS_sum_stats/quant_trait/gwas_loci_500kb/', 
...
for (i in 1:n_mod) {
  ...
  # pco_loci$snp = pqtl_maf_file$rsid[match(pco_loci$ID, pqtl_maf_file$ID)]
  if (length(pco_loci) < 1) {
    next
  }
  
  for (j in 1:nrow(gwas_loci)) {
    gwas_loci_j = fread(paste0('gtex/10_GWAS_coloc/gwas_loci/quant_trait/', gwas_trait, '/chr', 
    ...
    gwas_loci_j = gwas_loci_j[gwas_loci_j$SE > 0, ]
    gwas_loci_j = gwas_loci_j[gwas_loci_j$Freq_Tested_Allele > 0 & 
                                gwas_loci_j$Freq_Tested_Allele < 1, ]
    common_rsid = intersect(gwas_loci_j$POS, pco_loci$hg37_bp)
    if (length(common_rsid) > 0) {
      pco_loci_j = pco_loci[match(common_rsid, pco_loci$hg37_bp), ]
