...
ggplot() +
  geom_point(data = sun_z_ukb_extr, aes(x = rank, y = abs_z, color = cis_trans), size=1) +
  labs(x = "pQTL rank", y = '|Z|', title = 'Strongest cis- and trans-pQTLs effect size', color = "") +
...
ggplot() +
  geom_point(data = mrna_z_extr, aes(x = rank, y = abs_z, color = cis_trans), size=1) +
  labs(x = "pQTL rank", y = '|Z|', title = 'Strongest cis- and trans-pQTLs effect size', color = "") +
  scale_color_manual(values = brewer.pal(3,"Dark2")[1:2]) +
  theme(text = element_text(size=14, colour = "black"),
        axis.text.x = element_text(colour = "black", size = 13, angle = 0, hjust = 0.5),
...
n_rank_plot = nrow(sun_beta_ukb_extr)-10
cis_trans_ratio = data.frame(rank = rep(1:n_rank_plot, 2) ,
                             ratio = c(sort(sun_beta_ukb_extr$trans_z / sun_beta_ukb_extr$cis_z, decreasing = T)[1:n_rank_plot], 
                               sort(temp_trans/temp_cis, decreasing = T)[-(1:10)][1:n_rank_plot]),
                             cat = rep(c('protein', 'gene'), each = n_rank_plot))

ggplot() +
  geom_point(data = cis_trans_ratio, aes(x = rank, y = ratio, color = cat), size=1) +
  labs(x = "Ratio rank", y = '|trans Z / cis Z|', title = '', color = "") +
  scale_color_manual(values = brewer.pal(3,"Dark2")[1:2]) +
  theme(text = element_text(size=15, colour = "black"),
        axis.text.x = element_text(colour = "black", size = 13, angle = 0, hjust = 0.5),
        axis.text.y = element_text(colour = "black", size = 13),
...
dgn_cis = fread('/project/xuanyao/jinghui/gtex/06_qtl_z/dgn_cis/all.cis_gene.txt.gz')
dgn_beta = abs(dgn_cis$slope / dgn_cis$slope_se)
dgn_beta = sort(dgn_beta, decreasing = T)
sort_gene_cis = sort(temp_cis, decreasing = T)
sort_gene_cis[1:22] = dgn_beta[1:22]
sort_gene_trans = sort(temp_trans, decreasing = T)
n_rank_plot = nrow(sun_beta_ukb_extr)-10
cis_trans_ratio = data.frame(rank = rep(1:n_rank_plot, 2) ,
                             ratio = c(sort(sun_beta_ukb_extr$trans_z , decreasing = T)[1:n_rank_plot] / 
                                         sort(sun_beta_ukb_extr$cis_z, decreasing = T)[1:n_rank_plot], 
                                       sort_gene_trans[1:n_rank_plot]/sort_gene_cis[1:n_rank_plot]),
                             cat = rep(c('protein', 'gene'), each = n_rank_plot))

ggplot() +
  geom_point(data = cis_trans_ratio, aes(x = rank, y = ratio, color = cat), size=1) +
  labs(x = "QTL rank", y = '|trans Z / cis Z|', title = '', color = "") +
  scale_color_manual(values = brewer.pal(3,"Dark2")[1:2]) +
  theme(text = element_text(size=15, colour = "black"),
        axis.text.x = element_text(colour = "black", size = 13, angle = 0, hjust = 0.5),
        axis.text.y = element_text(colour = "black", size = 13),
...
cis_trans_ratio = data.frame(rank = rep(1:n_rank_plot, 2) ,
                             ratio = c(sort(sort(sun_beta_ukb_extr$trans_z , decreasing = T)[1:n_rank_plot] / 
                                         sort(sun_beta_ukb_extr$cis_z, decreasing = T)[1:n_rank_plot], decreasing = T), 
                                       sort(sort_gene_trans[1:n_rank_plot]/sort_gene_cis[1:n_rank_plot], decreasing = T)),
                             cat = rep(c('protein', 'gene'), each = n_rank_plot))
