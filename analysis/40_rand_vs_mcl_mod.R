...
# colnames(pco_rand) = c('chr', 'bp_hg37', 'mod', 'pco_p', 
#                        'loci', 'loci_start', 'loci_end', 'univar_max_logP')
rand_mod = fread('pqtl/11_prot_complex/random_mod.txt')
rand_mod$mod = paste0('mod', rand_mod$mod)
# mod_size = table(rand_mod$mod)
# pco_rand$size = mod_size[match(pco_rand$mod, names(mod_size))]
...
# total_nov_ppi[[4]] = total_nov_ppi[[4]][-which.max(total_nov_ppi[[4]])]
# total_nov_ppi[[5]] = total_nov_ppi[[5]][-which.max(total_nov_ppi[[5]])]
# total_nov_rand[[6]] = total_nov_rand[[6]][-which.max(total_nov_rand[[6]])]
# total_nov_rand[[8]] = total_nov_rand[[8]][-which.max(total_nov_rand[[8]])]
